...
class Trainer:
    ...
    def train(self, train_loader, optimizer, scheduler, wandb, epoch):
        ...
        for curr_iter, batch in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            labels, weights, scale_factors, affine, inverse = batch['label'].type(torch.LongTensor).to(self.device), \
            ...
            if self.cfg.DATA.DIMENSION == 3:
                ...
                labels = self.model.crop_vol_to_patch(labels)
                weights = self.model.crop_vol_to_patch(weights)
    ...
    @torch.no_grad()
    def final_eval(self, val_loader, meta):
        # Load best checkpoint
        optimizer = get_optimizer(self.model, self.cfg)
        ckpt_epoch, ckpt_miou = cp.load_from_checkpoint(
            os.path.join(self.checkpoint_dir, "Best_training_state.pkl"),
            self.model,
            optimizer,
            get_lr_scheduler(optimizer, self.cfg),
            self.cfg.TRAIN.FINE_TUNE)

        logger.info(f"Calculating metrics for best model at epoch {ckpt_epoch}")
        self.model.eval()

        # Set up iterators, metric arrays and labels
        label_list = self.lut["MappedID"][1:].values if self.cfg.DATA.PLANE != "sagittal" else self.lut["SagID"][1:].values
        metric_store = defaultdict(lambda: defaultdict(lambda: defaultdict))
        slicepersubject = meta["NumberSlices_" + self.cfg.DATA.PLANE]
        pred_vol = torch.zeros((slicepersubject[0], 256, 256), dtype=torch.float).to(self.device)
        label_vol = torch.zeros((slicepersubject[0], 256, 256),
                                dtype=torch.float)  # Label can be on cpu (avoid copying of data)

        i, start_index = 0, 0

        for curr_iter, batch in tqdm(enumerate(val_loader), total=len(val_loader)):

            images, scale_factors, labels, affine = batch[self.imgt].to(self.device), \
                                                                 batch['scale_factor'].to(self.device), \
                                                                 batch['label'], \
                                                                 batch["affine"]

            pred = self.model(images, scale_factors, affine=affine, inverse=affine,
                              scale_factor_out=None)

            _, pred = torch.max(pred, dim=1)

            b = pred.shape[0]
            shape_left = pred_vol[start_index:start_index + b, ...].shape[0]
            pred_vol[start_index:start_index + b, ...] += pred[:shape_left, ...]

            # Assemble label into volume
            label_vol[start_index:start_index + b, ...] = labels[:shape_left, ...]

            # Increase start index by shape of prediction
            start_index += b

            if start_index >= slicepersubject[i]:
                # Image finished, calculate and store results, reset pred_prob
                sid_with_res = meta["SubjectWithRes"][i]
                metric_dict = evaluate_metrics(label_vol, pred_vol.cpu(), label_list, self.combined_labels)
                metric_store["dice_ValSet"][sid_with_res] = metric_dict["dice"]
                metric_store["surfaceAverageHausdorff_ValSet"][sid_with_res] = metric_dict["surfaceAverageHausdorff"]

                if i == meta.shape[0] - 1:  # Ran over all images; finish analysis
                    break
                # Increase size and create new arrays
                i += 1
                pred_vol = torch.zeros((slicepersubject[i], 256, 256), dtype=torch.float).to(self.device)
                label_vol = torch.zeros((slicepersubject[i], 256, 256),
                                        dtype=torch.float)  # Label can be on cpu (avoid copying of data)

                # Add overlapping info (if mixed batch)
                start_index = 0 + b - shape_left
                pred_vol[0:start_index, ...] += pred[shape_left:, ...]
                label_vol[0:start_index, ...] += labels[shape_left:, ...]

        # Write scores to file
        colnames = self.lut["LabelName"][1:]
        if self.cfg.DATA.COMBINED_LABELS:
            colnames = np.hstack((colnames, self.combined_names))
        oname = "_" + self.cfg.EXPR_NUM + "_" + self.imgt + ".tsv"
        for metric in metric_store.keys():
            df = pd.DataFrame.from_dict(metric_store[metric], orient="Index", columns=colnames)
            df = df.reset_index()
            df[["SubjectName", "SF"]] = df["index"].str.split("+", 1, expand=True)
            # df[["SubjectName", "SF"]] = df.index.str.split("+").tolist() --> this gives and error for some reason?
            df.drop("index", axis=1).to_csv(os.path.join(self.cfg.LOG_DIR, "eval_metrics", metric + oname), sep="\t",
                                            index=False)
        return
    ...
    def run(self):
        ...
        # In the end, load the best checkpoint and get validation measures and plot for it.
        self.final_eval(val_loader, self.meta)
        if self.mix:
            # If mix, run eval on images of other type as well
            self.imgt = "image" if self.imgt == "t2_image" else "t2_image"
            self.final_eval(val_loader, self.meta)
        #if start_epoch <= self.cfg.TRAIN.NUM_EPOCHS:
            #wandb.finish()
