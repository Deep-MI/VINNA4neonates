...
class FastSurferCNNdoubleDB(FastSurferCNNBase):
    def __init__(self, params, padded_size):
        num_c = params["num_channels"]

        # Define FastSurferCNNBase network
        params["num_channels"] = params["num_filters_interpol"]
        super(FastSurferCNNdoubleDB, self).__init__(params)

        # Define Input Dense Block (IDB) equivalent to FlexSurfer
        params["num_channels"] = num_c
        self.inp_block = sm.InputDenseBlock(params)

        # Define Dense Output Block equivalent to FlexSkipSurfer
    ...
    def forward(self, x, scale_factor=None, affine=None, inverse=None, scale_factor_out=None):
        ...
        # FastSurfer baseline net (no 1x1 conv at end)
        decoder_output_0 = super().forward(skip_encoder_0, scale_factor)

        # Output Dense Block
        outblock = self.outp_block(decoder_output_0, skip_encoder_0)

        # Final logits layer (1x1 conv + softmax in loss)
        logits = self.classifier.forward(outblock)

        return logits


class FlexFastSurferCNN(FastSurferCNNBase):
    def __init__(self, params, padded_size):
        num_c = params["num_channels"]

        #params["num_filters_interpol_last"] = params["num_filters_interpol"] * 2 if params["smooth"] else params[
        #    "num_filters_interpol"]
        params["num_channels"] = params["num_filters_interpol"]

        # Base FastSurfer Network
        super(FlexFastSurferCNN, self).__init__(params)

        # Interpolation block options
        ...
        self.interpol1 = il.Affine2d((self.width, self.height),
                                   ...
                                   device=params["device"], elastic=params["elastic_deformation"])
    ...
    def forward(self, x, scale_factor, affine=None, inverse=None, scale_factor_out=None):
        ...
        # interpolation to 1mm (or whatever is encoded in scale_factor)
        x, corr_scale_factor = self.interpol1(x, scale_factor, affine)

        # FastSurfer baseline net (no 1x1 conv at end)
        net_out = super().forward(x, scale_factor=scale_factor)

        # Interpolation back to original resolution (or whatever is encoded in scale_factor)
        if scale_factor_out is None:
            scale_factor_out = corr_scale_factor
        else:
            scale_factor_out = np.round(np.asarray(scale_factor_out) * np.asarray(corr_scale_factor)
                                        / np.asarray(scale_factor.cpu().numpy()), 3)

        output, sf = self.interpol2(net_out, scale_factor_out, inverse, rescale=True)
...
_MODELS = {
    ...
    "FastSurferDDB": FastSurferCNNdoubleDB,
    "FastSurferSVINN": FlexFastSurferCNN,
